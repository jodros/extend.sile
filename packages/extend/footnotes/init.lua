...
function package:_init (options)
  ...
  if not SILE.scratch.counters.footnote then
...
function package:registerCommands ()
  ...
  self:registerCommand("note", function (_, content)
      if not SILE.scratch.notes then 
        SILE.scratch.notes = {{}}
      end
      
      SILE.call("footnotemark")
      SILE.scratch.notes[SILE.scratch.counters.footnote.value] = content
      SILE.scratch.counters.footnote.value = SILE.scratch.counters.footnote.value + 1
  end)

  self:registerCommand("shownotes", function ()
      for index, note in ipairs(SILE.scratch.notes) do
          SILE.call("neverindent")
          SILE.typesetter:typeset(index .. ".")
          SILE.call("glue", {width = "5pt"})
          SILE.call("font:footnotes", {}, note)
          SILE.call("par")
      end
  end)
  ...
  self:registerCommand("footnote:rule", function (options, _) -- NOT MY CODE, STOLEN FROM OMIKHLEIA RESILIENT...
  ...
  self:registerCommand("footnote", function (options, content)
    ...
    if not options.norule then
      SILE.call("footnote:rule")
    end
