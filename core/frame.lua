...
SILE.framePrototype = pl.class({
    ...
    _init = function(self, spec, dummy)
        ...
        if not dummy then
            for method in pairs(alldims) do
                self.variables[method] = cassowary.Variable({
                    name = spec.id .. "_" .. method
                })
    ...
    reifyConstraint = function(self, solver, method, stay)
        ...
        constraint = SU.type(constraint) == "measurement" and constraint:tonumber() or
                         SILE.frameParser:match(constraint)
